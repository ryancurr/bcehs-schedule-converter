...
st.caption("Upload a BCEHS schedule (.xlsx). Click ACP or PCP to generate the populated template CSV.")

year = st.number_input(
    "Year for dates",
    min_value=2020,
    max_value=2100,
    value=dt.date.today().year,   # ✅ current year default
    step=1,
)
...
def run_conversion(label: str):
    if bcehs_file is None:
        st.warning("Please upload the BCEHS schedule (.xlsx) first.")
        return
...
extracted = extract_rows_from_workbook(xlsx_bytes, int(year), label)
        ...
        out_name = f"bcehs-populated-template_{label}.csv"
        out_csv = out_df.to_csv(index=False).encode("utf-8")
        st.download_button(
            f"Download {label} populated template CSV",
            data=out_csv,
            file_name=out_name,
            mime="text/csv",
        )
        ...
        if debug:
            dbg_name = f"bcehs-debug_{label}.csv"
            dbg_csv = debug_df.to_csv(index=False).encode("utf-8")
            st.download_button(
                f"Download {label} debug CSV",
                data=dbg_csv,
                file_name=dbg_name,
                mime="text/csv",
            )
        ...
        st.error(f"{label} conversion failed.")
        st.exception(e)
